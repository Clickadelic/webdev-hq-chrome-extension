export default defineBackground(() => {
  chrome.runtime.onInstalled.addListener(() => {
    console.log(
      chrome.i18n.getMessage(
        "console_log_on_installed",
        "WebDev HQ Chrome-Extension installed."
      )
    )
  ...
  async function fetchSeasonalImage(): Promise<any> {
    ...
    try {
      const data = await getFromStorage<StorageData>([
        "seasonalImageResponse",
        "lastFetchedDate"
      ])
      if (data.seasonalImageResponse && data.lastFetchedDate === today) {
        ...
        const res = await fetch(
          `${import.meta.env.WXT_API_URL}/common/v1/chrome-extension/seasonal-image`
        )
