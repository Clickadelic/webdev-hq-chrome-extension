...
function Todos() {
    ...
    function renderTodoList(todoList) {
        todoList.forEach(item => {
            return React.createElement("li", { key: item.index }, item.todo);
        });
    }
    useEffect(() => {
        renderTodoList(todoList);
    ...
    return (React.createElement("div", { className: "todo-app m-auto md:w-[760px] justify-between rounded bg-white/10 backdrop backdrop-blur p-2" },
        React.createElement("form", { className: "bg-white-10 rounded flex justify-between mb-2", onSubmit: addTodo },
            React.createElement("input", { type: "text", name: "todo", id: "todo", className: "p-2 pl-4 text-base rounded w-full mr-2", onChange: handleChange, placeholder: chrome.i18n.getMessage("newTodo") }),
            React.createElement("span", { className: "inline-flex justify-between" },
                React.createElement("button", { onSubmit: addTodo, className: "bg-blue-600 text-white rounded p-3 hover:bg-blue-500" },
        ...
        React.createElement("ul", { className: "todo-list" }, todoList)));
}
