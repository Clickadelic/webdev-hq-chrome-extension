...
function GoogleApps() {
    const currentApps = apps.map(app => {
        return (React.createElement("li", { key: app.id },
            React.createElement("a", { href: app.href, title: app.title, target: app.target, className: "w-16 h-16 flex flex-col justify-center items-center backdrop-blur-sm bg-white/30 hover:bg-white hover:glow rounded-lg" },
                React.createElement("img", { src: app.icon, className: "w-8 h-8", alt: app.title }),
                React.createElement("span", { className: "text-sm mt-1 text-white hover:text-slate-600" }, app.title))));
    });
